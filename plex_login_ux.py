...
class Plex:
	...
	environment		accepted options are Classic and UX. Determines how the program will log in
	...
	db				Optional. Default to 'test'. Accepted values are 'test' and 'prod'. Can be changed via the config file after it is created.
	'''
	def __init__(self,environment, user_id, password, company_code, pcn='', db='test'):
...
Press OK to select the csv file.'''
		...
		while not pcn_path.is_file():
			confirm = messagebox.askokcancel(title='PCN file is missing', message=sql)
			if not confirm:
				messagebox.showinfo(title='User Selected Cancel', message='The program will now close.')
				sys.exit("Process quit by user")
			self.file_path = filedialog.askopenfilename()
			if self.file_path:
				self.csv_to_json(self.file_path)
		self.pcn_dict = {}
		with open('pcn.json', 'r', encoding='utf-8') as pcn_config:
			self.pcn_dict = json.load(pcn_config)
		current = set()
		# Sets the variables for the login operation based on UX or classic environment
	...
	'''
	Function to take a csv file from Plex and create a PCN JSON file that can be used to log into specific PCNs if the user has multiple PCN access.
	This should only be called on initialization if the pcn.json file does not exist yet.
	...
	'''
	Creates the config file which can be used to change any login details after it is created.
	'''
	def config(self):
	...
	If the config file is not present, it will get created with the data first used in the class call.
	Logs into Plex test/prod depending on the config
	Also will select the PCN if that is configured
	'''
	def login(self):
	# Using Chrome to access web
		extension_path = os.path.join(self.bundle_dir,'resources','cumulus_plugin.crx')
		executable_path = os.path.join(self.bundle_dir,'resources','chromedriver.exe')
	...
	# Default is test
		...
		driver.get('https://' + self.plex_db + self.plex_main)

	# Locate id and password fields
		id_box = driver.find_element_by_name(self.plex_log_id)
		pass_box = driver.find_element_by_name(self.plex_log_pass)
		company_code = driver.find_element_by_name(self.plex_log_comp)
	...
	# Click login
		login_button = driver.find_element_by_id(self.plex_login)
		login_button.click()
	...
	# By default this is configured to not be used
		...
		if self.pcn != '':
			if self.environment == 'UX':
				driver.get(f'{url_comb}/SignOn/Customer/{self.pcn}?{url_token}')
				return (driver, url_comb, url_token)
			else:
				self.pcn = self.pcn_dict[self.plex_pcn]
				driver.find_element_by_xpath('//img[@alt="' + self.pcn + '"]').click()
				return (driver, url_comb)

	'''
	Downloads the chromedriver and cumulus plugin that will allow selenium to function.
	If you do not use the cumulus plugin, you will need to do extra steps to control Plex after login using the pop-up window
	TODO - if the login is for UX, the Cumulus plugin isn't inherintly needed. Find a way to separate this out?
	'''
	...
	'''
	Pause function handling. Creates a pausefile.txt file in the working directory that can be used to pause the loop if the file exists.
	EX:
	Place the code snippet at the start of your loop. The current loop will complete after you pause the script.
	It will be paused until the file is deleted.
		# Handler to support pausing the script at the start of a loop. Useful for when you need to disconnect from the network for a short time.
